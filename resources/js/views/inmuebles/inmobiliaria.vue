<template>
    <div class="inmobiliaria">
        <ImageInmobiliaria :fondo="inmobiliaria?.fondo" />
        ...
        <div class="container">
            ...
            <div class="list-inmuebles col-md-9">
                ...
                <CardInmuebles v-for="vivienda in viviendas" :key="vivienda.id" :vivienda="vivienda" class="card-inmueble" />
            </div>
        </div>
    </div>
</template>
    ...
    setup() {
        ...
        const fetchInmobiliaria = async (id) => {
            try {
                ...
                // Fetch viviendas for the inmobiliaria's user ID
                fetchViviendasByUserId(inmobiliaria.value.id);
            } catch (error) {
                console.error('Error fetching inmobiliaria:', error);
            }
        };

        const fetchViviendasByUserId = async (userId) => {
            try {
                const response = await axios.get(`/api/viviendas/user/${userId}`);
                viviendas.value = response.data;
            } catch (error) {
                console.error('Error fetching viviendas by user ID:', error);
            }
        };
